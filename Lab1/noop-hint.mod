...
# restricciones de activacion para suma y producto
s.t. activacionProdSum {j in indices_jaulas_3}: aux_acti_sum[j] + aux_acti_prod[j] = 1;

# restricciones para las cotas de suma y producto
s.t. cotaSuma {j in indices_jaulas_3}: aux_cota_sum[j] <= M*(aux_acti_prod[j]);
s.t. cotaProd {j in indices_jaulas_3}: aux_cota_prod[j] <= M;
s.t. cotaSuma_inf {j in indices_jaulas_3}: aux_cota_sum[j] >= -M;
s.t. cotaProd_inf {j in indices_jaulas_3}: aux_cota_prod[j] >= -M;
...
# restricciones para las cotas

s.t. cotaResta {j in indices_jaulas_2}: aux_cota_rest[j] <= M*(1-aux_acti_rest[j]);
s.t. cotaDiv {j in indices_jaulas_2}: aux_cota_div[j] <= M*(1-aux_acti_div[j]);
s.t. cotaSuma2 {j in indices_jaulas_2}: aux_cota_sum[j] <= M*(1-aux_acti_sum[j]);
s.t. cotaProd2 {j in indices_jaulas_2}: aux_cota_prod[j] <= M*(1-aux_acti_prod[j]);

s.t. cotaResta_neg {j in indices_jaulas_2}: aux_cota_rest[j] >= -M*(1-aux_acti_rest[j]);
s.t. cotaDiv_neg {j in indices_jaulas_2}: aux_cota_div[j] >= -M*(1-aux_acti_div[j]);
s.t. cotaSuma2_neg {j in indices_jaulas_2}: aux_cota_sum[j] >= -M*(1-aux_acti_sum[j]);
s.t. cotaProd2_neg {j in indices_jaulas_2}: aux_cota_prod[j] >= -M*(1-aux_acti_prod[j]);
