...
var aux_div {j in {1..num_jaulas}} binary;
...
# restriccion para tipo 4
s.t. division {(i1,j1,i2,j2,j) in {1..N} cross {1..N} cross {1..N} cross {1..N} cross {1..num_jaulas}: (i1,j1) in {(p,z) in {1..N} cross {1..N}: declaracion[j,2]=4 and B[p,z] = j} and (i2,j2) in {(p,z) in {1..N} cross {1..N}: declaracion[j,2]=4 and B[p,z] = j} and ((i1=i2 and j1<j2) or (i1<i2))}: sum {k in {1..N}} x[i1,j1,k]*log(k)-sum {k in {1..N}} x[i2,j2,k]*log(k) = log(declaracion[j,1]) -2*aux_div[j]*log(declaracion[j,1]);